export const viz= (v)=> {
    ...
    const accessor = row => {
        ...
        if (row[8][0] === "$") {
            cleaned.push( //row[0],
                //row[2],
                row[4], row[7], +row[8].slice(2), +row[9].slice(2));
            return cleaned;
        } else if (row[6][0] === "$" && row[3] !== "") {
            cleaned.push( //row[0],
                //row[1],
                row[3], row[5], +row[6].slice(2), +row[7].slice(2));
            return cleaned;
        } else {
            return null;
        }
    ...
    (function(){
        let bP = {};
        let mainRectWidth = 30, buffMargin = 1, minHeight = 14;
        ...
        bP.partData = function (data, p) {
            ...
            let v1 = d3.set(data.map(function (d) { return d[0]; })).values();
            ...
            let v2 = d3.set(data.map(function (d) { return d[1]; })).values();
        ...
        function visualize(data) {
            ...
            function calculatePosition(a, s, e, b, m) {
                ...
                let scaleFact = leftoverHeight / Math.max(neededHeight, 1), sum1 = 0;

                ret.forEach(
                    function (d) {
                        d.percent = scaleFact * d.percent;
                        d.height = (d.height == m ? m : d.height * scaleFact);
                        d.middle = sum1 + b + d.height / 2;
        ...
        function drawPart(data, id, p) {
            d3.select("#" + id).append("g").attr("class", "part" + p).attr("transform", "translate(" + (p * (me.options.transitionWidth
                + mainRectWidth)) + ",0)");
            let el = d3.select("#" + id).select(".part" + p);
            el.append("g").attr("class", "subbars");
            el.append("g").attr("class", "mainbars");

            let mainbar = d3.select("#" + id).select(".part" + p).select(".mainbars").selectAll(".mainbar").data(data.mainBars[p])
        ...
        bP.selectSegment = function (data, m, s) {
            data.forEach(function (k) {
                ...
                let selectedBar = d3.select("#" + k.id).select(".part" + m).select(".mainbars").selectAll(".mainbar").filter(function (d, i) { return (i == s); });
    ...
    const drawD3Chart = (data) => {
        let newdata = [{
            data: bP.partData(data, 2),
            id: 'Sales',
            header: ["Channel", "State", "Sales"]
        }, {
            data: bP.partData(data, 3),
            id: 'Cost',
            header: ["Channel", "State", "Cost"]
        }];
        bP.setOptions({
            colors: ["#3366CC", "#DC3912", "#FF9900", "#109618", "#990099", "#0099C6"],
            labelColumn: [-195, 38],
            valueColumn: [-38, 149],
            //(count (%)) first value is left position x, second value is right position x
            barpercentColumn: [-3, 185],
        ...
        bP.draw(newdata, '#reportCsvExport');
    };
    ...
    const exportCsv = () => {
        report.export({
            ...
            request().done(parseData).fail(errorHandler);
        }, errorHandler);
        document.querySelector("#reportCsvExport #loader").style.display = "none";
    };
